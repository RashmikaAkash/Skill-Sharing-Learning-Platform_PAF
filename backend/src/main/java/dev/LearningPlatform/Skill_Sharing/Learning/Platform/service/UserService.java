    ...
    public User updateUser(String id, User details) {
        return userRepository.findById(id)
            .map(user -> {
                ...
                if (details.getProfilePhotoUrl() != null) user.setProfilePhotoUrl(details.getProfilePhotoUrl());
                if (details.getCoverPhotoUrl() != null) user.setCoverPhotoUrl(details.getCoverPhotoUrl());
